...
- [x] **Ring-overrun frame alignment** — on ring overrun the realtime callback's `push_slice` could push a partial frame, permanently rotating channel alignment even after the overrun cleared. Fixed: each push is clamped to the ring's free space rounded down to a whole frame (`push_len`, still lock- and allocation-free), with the remainder tallied as dropped.
- [x] **Make blocking commands `async`** — in Tauri 2, non-`async` commands run on the main thread; `start_capture` blocks on the engine's reply and `list_devices` with `includeAsio: true` loads ASIO drivers (can take seconds), freezing the webview meanwhile. Fixed: `list_devices` / `get_device_config` / `start_capture` / `stop_capture` are `#[tauri::command(async)]` and run on a background thread pool.
- [x] **Stale spectrum peak-hold across capture sessions** — `start()` reset `displayedPeak`/`clipLatched` but not `peaks`, so after Stop → Start the previous session's spectrum peak-hold line reappeared over the new capture. Fixed: `start()` clears `peaks`.
- [x] **Clip light can re-latch right after Reset** — a `meter-update` already in flight when Reset was pressed could still carry the old `truePeakMaxDb` and re-latch for one frame cycle. Fixed with an engine-side measurement `generation` on `Metrics` (bumped on configure/Reset); the UI ignores clip latching for frames at or below the generation last seen when Reset/Start was requested.
...
- [x] **Calibrated pink/brown noise reference curves** — shipped as the **Guide** dropdown: noise is synthesized in Rust and run through the same 96-band `spectrum()` pipeline as live audio (per sample rate, cached), so the curve shows what that noise actually displays as on this meter rather than an idealized −3/−6 dB/oct slope. The vertical anchor is the **Target loudness**: the same ebur128 instance measures the generated signal's integrated LUFS, and the curve is drawn offset by (target − measured) — exact, and it tracks the Target input live.
