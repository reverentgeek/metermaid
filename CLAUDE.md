...
CI (`.github/workflows/ci.yml`) runs `pnpm build`, `cargo fmt --check`, clippy, and `cargo test` on macOS + Linux (+ Windows for the ASIO path), plus a `cargo audit`. It is **path-aware**: a `changes` job classifies each push/PR (via an explicit `git diff` of the changed files) into three buckets: **site** (`site/**` plus the site-owned root files `netlify.toml` and `.github/workflows/site-deploy.yml`), **docs** (root/`docs/` markdown, `LICENSE*` — needs no build at all), and **app** (everything else: `src/`, `src-tauri/`, lockfiles, `third-party/`, the CI workflow itself, …). The diff is done by hand rather than a glob-filter action because "app = anything that isn't site or docs" can't be expressed as an OR-of-globs — a leading `**` always beats a later `!negation`. The app matrix + `audit` run only on app changes; a separate `build-site` job (Playwright suite on Node 24) runs only on site changes. So a website-only PR (and a docs-only PR) skips the whole Rust matrix, an app-only PR skips the site build, and a **docs-only PR skips both** (a `## [x.y.z]` `CHANGELOG.md` bump or a `CLAUDE.md`/`README.md` edit, even bundled into a website PR, won't spin up build-and-test). Only a genuinely empty diff falls back to running everything. Branch protection requires a single aggregate check, **`CI OK`** (the `ci-ok` job), rather than the individual jobs. `CI OK` always runs and passes only when every other job either succeeded or was intentionally skipped. The single gate is necessary because a skipped **matrix** job (`build-and-test` on a site-only change) collapses to one `build-and-test` check and never reports its per-OS contexts, so requiring `build-and-test (macos-latest)` directly would leave that context pending forever and block site-only PRs. (Skipped *non-matrix* checks like `audit` and `build-site` do satisfy their required context; the matrix legs do not.) Don't switch the path gate to a workflow-level `paths` filter either: that also leaves the check pending and blocks the merge. The pre-commit hook lints only *staged* `.rs`/`.ts`/`.md`; bypass with `git commit --no-verify`, skip just clippy with `SKIP_CLIPPY=1 git commit`.
