...
- Refreshed the Rust dependency tree to the latest compatible versions (`cargo update`). This clears RUSTSEC-2026-0221 (`event-listener` 5.4.1 → 5.4.2, an unsoundness allowing `!Send` tags to cross thread boundaries via `StackSlot`), and moves cpal 0.18.1 → 0.18.2, tauri-plugin-updater 2.10.1 → 2.11.0, tauri-plugin-opener 2.5.4 → 2.5.5, serde 1.0.229, and serde_json 1.0.151. `cargo audit` is clean.
- **Dropped the `windows-core = "0.61"` pin.** cpal 0.18.2 declares `windows` and `windows-core` as a matched 0.62 pair instead of the loose `>=0.61, <=0.62` range that let them drift apart — which is the resolution bug the pin was added for in 0.5.1. cpal's WASAPI backend now resolves both crates consistently on its own, so the pin no longer constrained anything; it only parked an unused `windows-core` 0.61 in the dependency graph. The tao/webview2-com side of the tree keeps its own matched `windows` 0.61.3 / `windows-core` 0.61.2 pair, unchanged. The base cpal requirement is raised from `0.18` to `0.18.2` to keep the fix from being resolved away.
- Refreshed the frontend lockfile, which clears GHSA-2v37-7h3g-55p8 (`nanoid` < 3.3.18, reached through Vite's PostCSS). Vite 8.2.0 → 8.2.2, Biome 2.5.7 → 2.5.12, @tauri-apps/plugin-updater 2.10.1 → 2.11.0, @tauri-apps/plugin-opener 2.5.4 → 2.5.5, and `biome.json`'s `$schema` migrated to match the installed Biome.
- Refreshed the website lockfile, clearing 12 advisories in Eleventy's build-time transitive tree (`js-yaml`, `brace-expansion`, `linkify-it`, `liquidjs` — all denial-of-service issues in parsing, none reachable at runtime since the site is statically generated). Playwright 1.61.1 → 1.63.0, Tailwind 4.3.1 → 4.3.3, eleventy-plugin-edgejs 1.1.14 → 1.1.25, markdown-it 14.2.0 → 14.3.1.
- Bumped the pinned pnpm release from 11.20.0 to 11.26.0 (`packageManager` + lockfile).
...
- **Calibrated noise guide curves.** A Guide dropdown draws what pink or brown noise at your Target loudness would look like on this meter — generated in Rust and measured through the same 96-band analyzer as live audio (not an idealized −3/−6 dB per octave line, which the peak-bin display would not match), anchored via the same signal's integrated LUFS and tracking the Target input live. The selection persists across launches. ([#9](https://github.com/reverentgeek/metermaid/issues/9))
