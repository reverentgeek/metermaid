...
- cpal 0.18.2 declares `windows` and `windows-core` as a matched 0.62 pair instead of the loose `>=0.61, <=0.62` range that let them drift apart, so its WASAPI backend now resolves both crates consistently on its own. The `windows-core = "0.61"` pin added in 0.5.1 is left in place for this release — it no longer constrains cpal, and removing it is a separate change to make once a Windows build has confirmed 0.18.2 is healthy.
